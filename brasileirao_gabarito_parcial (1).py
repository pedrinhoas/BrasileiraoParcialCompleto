...
def nome_campeao(dados):
    id_time = id_campeao(dados)
    nome_time = nome_do_time(dados, id_time)
    return nome_time
...
def qtos_libertadores(dados):
    string_libertadores = dados['fases']['2700']['faixas-classificacao']['classifica1']['faixa']
    lista = string_libertadores.split('-')
    return int(lista[1])

def ids_dos_melhor_classificados(dados,numero_de_times):
    todos_em_ordem = dados['fases']['2700']['classificacao']['grupo']['Único']
    resposta = []
    #for i in [0,1,2,3,4]: #range(5) [0,1,2,3,4]
                           #range(7) [0,1,2,3,4,5,6]
                           #range(3) [0,1,2]
    for i in range(numero_de_times):
        resposta.append(todos_em_ordem[i])
    return resposta

#ids de todos os classificados
# funcao da linha 132, usando o numero_de_times retornado pela funcao da linha 127
def classificados_libertadores(dados):
    ...
    #>>> quantidade = qtos_libertadores(dados2018)
    #>>> quantidade
    #6
    #>>> ids        = ids_dos_melhor_classificados(dados2018,quantidade)
    #>>> ids
    #['17', '1', '15', '13', '24', '4']
    quantidade = qtos_libertadores(dados)
    ids        = ids_dos_melhor_classificados(dados,quantidade)
    return ids

# chamar a funcao da linha 144, mas depois converter os nomes usando a funcao nome_do_time
def nomes_classificados_libertadores(dados):
    ids_dos_classificados = classificados_libertadores(dados)
    resposta = []
    for id_time in ids_dos_classificados:
        n = nome_do_time(dados,id_time)
        resposta.append(n)
    return resposta
